...
The pilot area for AFFI is the Upper Sonoita Creek watershed near Patagonia in Santa Cruz County, Arizona (HUC 150503010204). This drains 55.4 mi² (143.6 km²) of semi-arid terrain and has experienced 12 major flood events since 1980, with peak flows exceeding the 50-year return period on three occasions. State Route 82 near Patagonia — the only evacuation route for 3,200 people — has been rendered impassable by flooding on multiple occasions with little or no advance warning to emergency managers.
...
- **Watershed area / delineation (left unchanged).** The Problem Statement's "55.4 mi² (143.6 km²)" for the HUC-12 was kept. Note that `config/watersheds/upper_sonoita.yaml` uses `area_km2: 510` (≈197 mi²), and `ensemble.py` uses 510 km² for the Sonoita basin — these describe the larger gauge-drainage area, not the HUC-12 sub-watershed. I did not reconcile this because I could not confirm which delineation you intend for the paper. **Please confirm which area figure is correct for the pilot description.**
